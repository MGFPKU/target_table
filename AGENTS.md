...
**Data flow:**
1. `fetch_raw_data()` in `data.py` downloads `China_Climate_Target_Tracker_cn.xlsx` / `China_Climate_Target_Tracker_en.xlsx` from the latest GitHub release of `MGFPKU/target_dataset` (or reads a local file).
...
`sheets.json` lists the source sheets inside the dataset files (`China_Climate_Target_Tracker_cn.xlsx` / `China_Climate_Target_Tracker_en.xlsx`) that are read and concatenated. The "Energy|Power" entry covers two identically-structured sheets that `pl.read_excel` treats as a single sheet name.
